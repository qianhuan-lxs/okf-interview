#!/usr/bin/env python3
"""Split the combined skill-meaning-loading-evolution.md into 3 atomic IKF docs."""
...
# 1) 含义 / 为什么不只是 system prompt
q("ml-ai/agent/skill-what-and-why.md",
  "Skill 是什么 / 包含什么 / 为什么不只是 system prompt",
  "ml-ai", "agent", "medium",
  ["skill", "prompt-engineering", "progressive-disclosure"],
  ALL,
  """# Skill 是什么 / 包含什么 / 为什么不只是 system prompt
...
## 延伸
""",
  languages=[], role=["ai-app", "sde", "backend"],
  source="_interviews/2026-05-louis-ai-java", status="reviewed",
  links=["ml-ai/agent/skill-loading-mechanism",
         "ml-ai/agent/skill-self-evolution-hermes",
         "ml-ai/mcp/mcp-vs-skill-difference"])

# 2) 加载机制 / two-stage 路由
q("ml-ai/agent/skill-loading-mechanism.md",
  "Skill 何时调用哪个 / 加载机制 (two-stage 路由)",
  "ml-ai", "agent", "medium",
  ["skill", "loading", "routing", "progressive-disclosure", "framework"],
  ALL,
  """# Skill 何时调用哪个 / 加载机制 (two-stage 路由)
...
""",
  languages=[], role=["ai-app", "sde", "backend"],
  source="_interviews/2026-05-louis-ai-java", status="reviewed",
  links=["ml-ai/agent/skill-what-and-why",
         "ml-ai/agent/skill-self-evolution-hermes",
         "ml-ai/agent/agent-function-call-mechanism"])

# 3) 自进化 / 自净化 —— Hermes
q("ml-ai/agent/skill-self-evolution-hermes.md",
  "Skill 自进化 / 自净化 (Hermes + GEPA)",
  "ml-ai", "agent", "hard",
  ["skill", "self-evolution", "hermes", "gepa", "dspy", "self-improving"],
  PURIFY,
  """# Skill 自进化 / 自净化 (Hermes + GEPA)
...
""",
  ...
  source="_interviews/2026-05-louis-ai-java", status="reviewed",
  links=["ml-ai/agent/skill-what-and-why",
         "ml-ai/agent/skill-loading-mechanism",
         "ml-ai/observability/claude-code-hooks-implementation",
         "ml-ai/mcp/mcp-vs-skill-difference"])

print("\nDone: split skill doc into 3 atomic docs")
