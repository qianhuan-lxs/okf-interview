...
1. 用 **Spring AI / LangChain4j reactive** —— `ChatClient.prompt().stream()` 已返回 `Flux<ChatResponse>`，直接 `map` 返回最自然，别强行转阻塞。
2. 服务本质是 **LLM 网关**（10k+ 并发流式代理）→ WebFlux + WebClient 透传 SSE（不解码不重编码）最省资源。
3. **已全栈 WebFlux** → 保持栈一致。
